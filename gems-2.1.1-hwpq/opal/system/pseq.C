...
pseq_t::~pseq_t() {
 ...
 if(m_arch_bpred)
  ...
  for(uint k=0; k < CONFIG_LOGICAL_PER_PHY_PROC; ++k) {
    if(m_stat_hwds_retired[k])
      delete [] m_stat_hwds_retired[k];
    if(m_stat_hwds_squashed[k])
      delete [] m_stat_hwds_squashed[k];
    if(m_stat_hwds_executed[k])
      delete [] m_stat_hwds_executed[k];
  }
  if(m_stat_hwds_retired)
    delete [] m_stat_hwds_retired;
  if(m_stat_hwds_squashed)
    delete [] m_stat_hwds_squashed;
  if(m_stat_hwds_executed)
    delete [] m_stat_hwds_executed;
...
//***************************************************************************
const char* pseq_t::stat_hwds_mnemonic( enum hwds_inst_type_t instruction ) {
  switch(instruction) {
  case HWDS_INST_UNUSED:
    return "unspecified";
  case HWDS_INST_FIRST:
    return "first";
  case HWDS_INST_ENQUEUE:
    return "enqueue";
  case HWDS_INST_EXTRACT:
    return "extract";
  case HWDS_INST_GET_CURRENT_SIZE:
    return "get_current_size";
  case HWDS_INST_GET_CONTEXT:
    return "get_context";
  case HWDS_INST_EXTRACT_LAST:
    return "extract_last";
  case HWDS_INST_SPILL:
    return "spill";
  case HWDS_INST_FILL:
    return "fill";
  case HWDS_INST_LAST_PRIO:
    return "last_prio";
  case HWDS_INST_GET_POINTER:
    return "get_pointer";
  case HWDS_INST_ADJUST_SPILL_COUNT:
    return "adjust_spill_count";
  case HWDS_INST_GET_SIZE:
    return "get_size";
  case HWDS_INST_SET_CURRENT_ID:
    return "set_current_id";
  case HWDS_INST_GET_CURRENT_ID:
    return "get_current_id";
  case HWDS_INST_SET_SIZE:
    return "set_size";
  case HWDS_INST_INVALIDATE:
    return "invalidate";
...
void pseq_t::resetStats_stat_hwds( void )
{
  for( uint k=0; k <  CONFIG_LOGICAL_PER_PHY_PROC; ++k) {
    for ( uint i = 0; i < HWDS_NUM_INST_TYPES; ++i ) {
      m_stat_hwds_retired[k][i] = 0;
      m_stat_hwds_squashed[k][i] = 0;
      m_stat_hwds_executed[k][i] = 0;
    }
  }
}
...
void pseq_t::printStats( void )
{
 ...
 for(uint k=0; k <  CONFIG_LOGICAL_PER_PHY_PROC; ++k){  
  ...
  // GAB: HWDS stats
  out_info("  %-50.50s %10llu\n", "hwds instrs committed:",
      m_stat_hwds_retired[k][0]);
  for (uint i = 1; i < HWDS_NUM_INST_TYPES; ++i) {
    char buf[101];
    sprintf(buf, "%s%s%s","hwds ",
        stat_hwds_mnemonic(static_cast<enum hwds_inst_type_t>(i)),
        " instrs committed:");
    out_info("  %-50.50s %10llu\n", buf, m_stat_hwds_retired[k][i]);
  }
  out_info("  %-50.50s %10llu\n", "hwds instrs squashed:",
      m_stat_hwds_squashed[k][0]);
  for (uint i = 1; i < HWDS_NUM_INST_TYPES; ++i) {
    char buf[101];
    sprintf(buf, "%s%s%s","hwds ",
        stat_hwds_mnemonic(static_cast<enum hwds_inst_type_t>(i)),
        " instrs squashed:");
    out_info("  %-50.50s %10llu\n", buf, m_stat_hwds_squashed[k][i]);
  }
  out_info("  %-50.50s %10llu\n", "hwds instrs executed:",
      m_stat_hwds_executed[k][0]);
  for (uint i = 1; i < HWDS_NUM_INST_TYPES; ++i) {
    char buf[101];
    sprintf(buf, "%s%s%s","hwds ",
        stat_hwds_mnemonic(static_cast<enum hwds_inst_type_t>(i)),
        " instrs executed:");
    out_info("  %-50.50s %10llu\n", buf, m_stat_hwds_executed[k][i]);
  }
...
pseq_t::initializeStats(void) {
 ...
 m_hist_smt_fetch_per_cycle = (uint64 *) malloc( sizeof(uint64) * (m_threads_per_cycle+1));
  ...
  m_stat_hwds_squashed = new uint64*[CONFIG_LOGICAL_PER_PHY_PROC];
  m_stat_hwds_executed = new uint64*[CONFIG_LOGICAL_PER_PHY_PROC];
  for(uint k=0; k < CONFIG_LOGICAL_PER_PHY_PROC; ++k){
    m_stat_hwds_retired[k] = new uint64[HWDS_NUM_INST_TYPES];
    m_stat_hwds_squashed[k] = new uint64[HWDS_NUM_INST_TYPES];
    m_stat_hwds_executed[k] = new uint64[HWDS_NUM_INST_TYPES];
  }
