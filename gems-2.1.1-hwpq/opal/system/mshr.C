 ...
 * Version:   $Id: mshr.C 1.37 03/07/10 18:07:48-00:00 milo@cottons.cs.wisc.edu $
 */
...
mshr_t::Miss(pa_t a, cache_t *c, mshr_priority_t p, waiter_t *w, bool *primary_bool)
{
  ...
  // GAB: force a miss
  #ifdef GAB_FORCE_MISS
    if ( 0 && (m = HashMatch(block_address)) != NULL ) {
  #else
...
mshr_t::IsAddressInCache(pa_t addr) {
  ...
  // GAB: check for null here... something wrong with mshr setup?
  return ((m_cache_fetch_from != NULL && m_cache_fetch_from->TagCheck(ba))
      || HashMatch(ba) != NULL);
}
