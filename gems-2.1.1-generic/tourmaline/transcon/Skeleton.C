 ...
 * See Skeleton.h
 * 
 * $Id: Skeleton.C 1.00 05/11/09 14:02:03-05:00 gibson@mustardseed.cs.wisc.edu $
 *
 */
...
Skeleton::Skeleton() : TransactionController() {

  /* All Transaction Controllers Should Execute the following code */
  /* [Begin] */
        
  m_type = Transaction_Controller_FARFO;  // Substitute proper enum here (define in TransactionControllerTypes.h)
...
void Skeleton::beginTransaction(int proc, int xactno) {
...
void Skeleton::endTransaction(int proc, int xactno) {
...
void Skeleton::makeRequest(const MemoryAccess& access) {
 assert(0);
}

void Skeleton::finishRequest(MemoryAccess& access) {
 assert(0);
}
